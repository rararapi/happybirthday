  ...
  return (
    ...
    <group position={[0, -2.05, 0]}>
      ...
      <mesh position={[0, 0, 0]}>
        ...
        <meshStandardMaterial color="#7B4A1E" roughness={0.75} metalness={0.05} />
      </mesh>

      {/* テーブルクロス（天板より少し大きく） */}
      <mesh position={[0, 0.10, 0]}>
        <cylinderGeometry args={[2.45, 2.45, 0.06, 48]} />
        <meshStandardMaterial color="#fffbf4" roughness={0.9} />
      </mesh>

      {/* テーブルクロス垂れ下がり */}
      <mesh position={[0, -0.38, 0]}>
        <cylinderGeometry args={[2.44, 2.44, 0.75, 48, 1, true]} />
        <meshStandardMaterial color="#fffbf4" roughness={0.9} side={2} />
      </mesh>
      ...
      {legPositions.map(([x, y, z], i) => (
        <mesh key={i} position={[x, y, z]}>
          ...
          <meshStandardMaterial color="#5C3510" roughness={0.8} />
        </mesh>
      ))}
    </group>
  )
